...
public class DoubanApiClient : IMovieApiClient
{
    ...
    private static MovieSearchResult ParseDetail(string html, string id)
    {
        ...
        // 导演：匹配所有 rel="v:directedBy" 的人物链接，允许 <a> 内有嵌套标签
        var dirNames = new List<string>();
        foreach (Match dm in Regex.Matches(html, @"rel=""v:directedBy""[^>]*>(.*?)</a>", RegexOptions.Singleline))
        {
            var dir = CleanDirector(dm.Groups[1].Value);
            if (!IsTemplateOrLabel(dir) && !dirNames.Contains(dir))
                dirNames.Add(dir);
        }
        // fallback：从 info 区域 "导演" 文本附近提取人名
        if (dirNames.Count == 0)
        {
            var infoDm = Regex.Match(html, @"导演[：:]\s*<[^>]+>\s*<a[^>]*>(.*?)</a>", RegexOptions.Singleline);
            if (!infoDm.Success) infoDm = Regex.Match(html, @"导演[：:]\s*<a[^>]*>(.*?)</a>", RegexOptions.Singleline);
            if (infoDm.Success)
            {
                var dir = CleanDirector(infoDm.Groups[1].Value);
                if (!IsTemplateOrLabel(dir)) dirNames.Add(dir);
            }
        }
        if (dirNames.Count > 0) r.Director = string.Join(" / ", dirNames.Take(3));
        else r.Director = CleanDirector(Regex.Match(html, @"导演[：:]\s*([^<\n]+)").Groups[1].Value.Trim());
        var cl = Regex.Matches(html, @"rel=""v:starring""[^>]*>([^<]+)</a>").Select(c => StripHtml(c.Groups[1].Value.Trim())).Where(c => !IsTemplateOrLabel(c)).Take(8).ToList(); if (cl.Any()) r.Cast = string.Join(", ", cl);
