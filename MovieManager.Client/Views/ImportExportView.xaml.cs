...
public partial class ImportExportView : UserControl
{
    ...
    private async void BackupDbFile_Click(object sender, RoutedEventArgs e) { var d = new SaveFileDialog { Filter = "DB|*.db", FileName = $"MovieManager_{DateTime.Now:yyyyMMdd_HHmm}.db" }; if (d.ShowDialog() != true) return; try { await _importExportService.BackupDatabaseAsync(d.FileName); Log("OK"); } catch (Exception ex) { Log(ex.Message); } }
