...
public class DoubanApiClient : IMovieApiClient
{
    ...
    public static string ExtractChineseKeyword(string title)
    {
        ...
        var chinese = Regex.Match(title, @"[\u4e00-\u9fff]+");
        return chinese.Success ? chinese.Value : title.Split(' ')[0].Trim();
    }
    ...
    public static string? ExtractEnglishHint(string title)
    {
        ...
        // 去掉中文部分和常见编码标记
        var cleaned = Regex.Replace(title, @"[\u4e00-\u9fff]+", " ");
        ...
        cleaned = Regex.Replace(cleaned, @"\b\d{4}\b", " "); // 年份
        ...
        if (idx >= 0)
        {
            ...
            try
            {
                ...
                foreach (var item in doc.RootElement.GetProperty("items").EnumerateArray())
                {
                    ...
                    { if (part.EndsWith("分钟") && int.TryParse(part.Replace("分钟", ""), out var rt)) runtime = rt; else if (part is "美国" or "中国" or "日本" or "韩国" or "英国" or "法国" or "德国" or "印度" or "加拿大" or "意大利" or "西班牙" or "俄罗斯" or "泰国" or "香港" or "台湾") country = part; }
                    ...
                    foreach (var part in abs.Split(" / ").Select(p => p.Trim()))
                    {
                        ...
                        if (part.Any(c => c >= 0x4e00 && c <= 0x9fff)) continue; // 含中文跳过
    ...
    private static MovieSearchResult ParseDetail(string html, string id)
    {
        ...
        var pm = Regex.Match(html, @"<img[^>]*src=""(https://img\d+\.doubanio\.com/view/photo/[^""]+)"""); if (pm.Success) r.PosterUrl = pm.Groups[1].Value;
        return r;
    }
}
